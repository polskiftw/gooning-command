...
The `POST /api/sources` endpoint uses a full-page, same-origin form submission so browsers perform the mTLS handshake for the source change instead of relying on a background request. It still requires a successfully verified, non-revoked client certificate. It accepts either an exact same-origin `Origin` header or, for Safari form navigations that omit `Origin`, the browser-controlled `Sec-Fetch-Site: same-origin`, `Sec-Fetch-Mode: navigate`, and `Sec-Fetch-Dest: document` combination. The endpoint accepts only one small URL-encoded `subreddit` field, validates and normalizes the name, rejects malformed input, ignores case-insensitive duplicates, and uses an ETag-protected R2 write so simultaneous additions cannot silently overwrite each other. Results return through a `303` redirect containing only a fixed status code. The private source object is outside `gallery/`, is absent from `gallery-index.json`, and cannot be served by the media route. Yoink reads it on the next scheduled or manual run.
