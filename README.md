...
The `POST /api/sources` endpoint uses a full-page, same-origin form submission so browsers perform the mTLS handshake for the source change instead of relying on a background request. It still requires a successfully verified, non-revoked client certificate. The certificate-protected page issues a random 256-bit CSRF token in a host-only `Secure`, `HttpOnly`, `SameSite=Strict` cookie and embeds the matching value in the form. The endpoint requires both values to match. It deliberately does not authenticate form submissions with `Origin` or Fetch Metadata because Safari's advanced privacy modes may omit or rewrite those optional headers; the verified certificate and double-submit token remain authoritative. The endpoint accepts only one small URL-encoded `subreddit` field, validates and normalizes the name, rejects malformed input, ignores case-insensitive duplicates, and uses an ETag-protected R2 write so simultaneous additions cannot silently overwrite each other. Results return through a `303` redirect containing only a fixed status code. The private source object is outside `gallery/`, is absent from `gallery-index.json`, and cannot be served by the media route. Yoink reads it on the next scheduled or manual run.
