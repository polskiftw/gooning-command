...
def load_settings() -> dict[str, Any]:
    ...
    browser_user_agent = str(settings.get("browser_user_agent", "")).strip()
    if not browser_user_agent:
        raise RuntimeError(
            "settings.json is missing browser_user_agent. Paste your browser's complete User-Agent string."
        )
    if "PASTE_YOUR" in browser_user_agent.upper() or "CHANGE_THIS" in browser_user_agent.upper():
        raise RuntimeError(
            "browser_user_agent still contains the placeholder. Replace it with your browser's complete User-Agent string."
        )
    if "\n" in browser_user_agent or "\r" in browser_user_agent:
        raise RuntimeError("browser_user_agent must be one single line")
    if posts < 1:
...
def build_gallery_dl_config(settings: dict[str, Any]) -> None:
    ...
    browser_user_agent = str(settings["browser_user_agent"]).strip()
    ytdl_headers = {"User-Agent": browser_user_agent}
    config = {
        "extractor": {
            ...
            "ytdl": {
                "enabled": True,
                "raw-options": {"http_headers": ytdl_headers},
            },
        },
        "downloader": {
