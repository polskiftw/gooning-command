...
def scan_once(settings: dict[str, Any], client, bucket: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gooning-party-") as tmp:
        ...
        for number, source in enumerate(settings["sources"], start=1):
            ...
            archive_checkpoint = DATA_DIR / f"archive-before-source-{number}.sqlite3"
            if ARCHIVE_PATH.exists():
                archive_checkpoint.write_bytes(ARCHIVE_PATH.read_bytes())
            else:
                archive_checkpoint.unlink(missing_ok=True)
            ...
            if return_code == 0 and failed_uploads == 0:
                clean_successes += 1

            if failed_uploads:
                # gallery-dl has already marked downloaded files in its archive. Roll
                # this source back so failed uploads are downloaded again next run.
                if archive_checkpoint.exists():
                    ARCHIVE_PATH.write_bytes(archive_checkpoint.read_bytes())
                else:
                    ARCHIVE_PATH.unlink(missing_ok=True)
                log.warning(
                    "%s upload(s) failed for this source. Rolled back its download "
                    "history so the files will be retried next run.",
                    failed_uploads,
                )
            else:
