...
from deduper.family_recertification_queue import PendingFamilyRecertification
from deduper.family_recertification_status import family_recertification_status_text


class FamilyRepairStatusTests(unittest.TestCase):
    def job(self, status: str, attempts: int, error: str | None = None):
        return PendingFamilyRecertification(
            recertification_id=7,
            deleted_key="A",
            protected_key="1",
            priority_keys=("1", "2"),
            status=status,
    ...
    def test_pending_status_names_deleted_and_protected_objects(self):
        text = family_recertification_status_text(self.job("pending", 0))
    ...
    def test_running_status_is_literal(self):
        text = family_recertification_status_text(self.job("running", 1))
        self.assertIn("Family recertification running", text)
    ...
    def test_retry_status_includes_attempts_and_saved_failure(self):
        text = family_recertification_status_text(self.job("retry", 2, "network timeout"))
