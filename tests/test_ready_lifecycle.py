...
class ReadyLifecycleTests(unittest.TestCase):
    ...
    def test_session_exclusion_is_forgotten_on_rebuild(self) -> None:
        self.database.replace_pairs([("a.jpg", "b.jpg", 90.0, "first")])
        self.assertTrue(self.database.exclude_pair_keys("a.jpg", "b.jpg"))
        self.assertEqual(self.database.queued_deletions(), [])

        self.database.replace_pairs([("a.jpg", "b.jpg", 91.0, "next run")])
        rebuilt = self.database.scan_pairs()[0]
        self.assertNotEqual(rebuilt.status, "excluded")
        self.assertEqual(len(self.database.queued_deletions()), 1)

    def test_permanent_exclusion_survives_rebuild_and_orientation_change(self) -> None:
        self.database.replace_pairs([("a.jpg", "b.jpg", 90.0, "first")])
    ...
    def test_permanent_exclusion_survives_database_reopen(self) -> None:
        self.database.replace_pairs([("a.jpg", "b.jpg", 90.0, "first")])
        self.database.exclude_pair_permanently("a.jpg", "b.jpg")
        self.database.close()
    ...
    def test_deleted_asset_purges_obsolete_permanent_decision(self) -> None:
        self.database.replace_pairs([("a.jpg", "b.jpg", 90.0, "first")])
        self.database.exclude_pair_permanently("a.jpg", "b.jpg")
